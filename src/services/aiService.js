...
// comparisons below — cheap since inputs are individual words, never full sentences.
...
function fuzzyItemNameMatch(target, candidate) {
  ...
  if (condensedTarget.length >= 5 && (candidateWords.length ? condensedCandidate.includes(condensedTarget) || condensedTarget.includes(condensedCandidate) : false)) {
    return true;
  }
...
// Exact normalized match first, then containment either direction, then a fuzzy
// fallback for OCR/handwriting noise, else null (unmatched).
function matchPoItem(rawName, poItems) {
  ...
  for (const poItem of poItems) {
    const candidate = normalizeItemName(poItem.item_name);
    if (candidate && fuzzyItemNameMatch(target, candidate)) return poItem;
  }
  return null;
}
...
function compareQuoteToRequestedItems(quoteItems = [], requestedItems = []) {
  ...
  const comparisons = requested.map((req) => {
    ...
    // Fallback for OCR/handwriting noise (plurals, spacing, single-letter misreads) —
    // only runs when exact match and plain substring containment both found nothing.
    if (!matched) {
      for (const qi of quoteItems) {
        if (usedQuoteItemIds.has(qi.id)) continue;
        const candidate = normalizeItemName(qi.item_name_raw);
        if (candidate && fuzzyItemNameMatch(target, candidate)) { matched = qi; break; }
      }
    }
