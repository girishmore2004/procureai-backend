...
exports.getSpend = asyncHandler(async (req, res) => {
  ...
  const rows = await sequelize.query(query, { replacements: { companyId: req.companyId }, type: QueryTypes.SELECT });
  okResponse(res, rows);
});
...
exports.getDashboardKpis = asyncHandler(async (req, res) => {
  ...
  okResponse(res, {
    open_requests: parseInt(pr[0]?.open_requests || 0),
    pending_approvals: parseInt(ap[0]?.pending_approvals || 0),
    awaiting_quotes: parseInt(rfq[0]?.awaiting_quotes || 0),
    reorder_alerts: parseInt(inv[0]?.reorder_alerts || 0),
    invoice_mismatches: parseInt(inv_mm[0]?.invoice_mismatches || 0),
  });
});
