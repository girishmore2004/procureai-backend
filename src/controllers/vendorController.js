...
//   const allowed = ['name', 'legal_name', 'contact_person', 'email', 'phone', 'whatsapp_number', 'address', 'gstin', 'categories', 'payment_terms', 'lead_time_days', 'moq', 'preferred', 'notes', 'status', 'bank_account_number', 'bank_ifsc', 'bank_name', 'upi_id'];
//   allowed.forEach((f) => { if (req.body[f] !== undefined) vendor[f] = req.body[f]; });
//   await vendor.save();
//   await audit({ companyId: req.companyId, userId: req.user.id, action: 'vendor.updated', entityType: 'Vendor', entityId: vendor.id, before, after: vendor.toJSON(), ip: req.ip });
//   okResponse(res, vendor);
...
exports.list = asyncHandler(async (req, res) => {
  const { page, perPage, limit, offset } = paginate(req.query);
  // Two sources of vendors show up here side by side:
  //  1. Vendors this company added directly (company_id = this company).
  //  2. Vendors who self-registered via the Vendor Portal (company_id = null,
  //     "public"/marketplace vendors) — these were previously excluded
  //     entirely, so a self-signed-up vendor could never be found or added
  //     to an RFQ/invoice from the buyer's UI. They're included here as
  //     read-only entries (the frontend disables edit/delete for them,
  //     since this company doesn't own that vendor record) but are fully
  //     usable everywhere a vendor can be selected, matching what the
  //     backend already allowed for RFQ attachment.
  const where = {
    [Op.or]: [{ company_id: req.companyId }, { company_id: null }],
    deleted_at: null,
  };
  if (req.query.status) where.status = req.query.status;
  if (req.query.preferred) where.preferred = req.query.preferred === 'true';
  if (req.query.category) where.categories = { [Op.contains]: [req.query.category] };
  if (req.query.search) where.name = { [Op.iLike]: `%${req.query.search}%` };
  // This company's own vendors first, then portal vendors, alphabetically within each group.
  const result = await Vendor.findAndCountAll({
    where, limit, offset,
    order: [[sequelize.literal('CASE WHEN "Vendor"."company_id" IS NULL THEN 1 ELSE 0 END'), 'ASC'], ['name', 'ASC']],
  });
  const data = result.rows.map((v) => {
    const json = v.toJSON();
    json.is_portal_vendor = json.company_id === null;
    return json;
  });
  paginatedResponse(res, { count: result.count, rows: data }, { page, perPage });
});
...
exports.getOne = asyncHandler(async (req, res) => {
  const { Op } = require('sequelize');
  const vendor = await Vendor.findOne({
    where: { id: req.params.id, [Op.or]: [{ company_id: req.companyId }, { company_id: null }] },
    include: [VendorDocument, VendorScore],
  });
  if (!vendor) return errorResponse(res, 'NOT_FOUND', 'Vendor not found', 404);
  // Recent POs — scoped to this company regardless of who owns the vendor
  // record, so a portal vendor's profile only shows orders this company placed.
  const recentPOs = await PurchaseOrder.findAll({ where: { vendor_id: vendor.id, company_id: req.companyId }, limit: 10, order: [['created_at', 'DESC']] });
  okResponse(res, { ...vendor.toJSON(), is_portal_vendor: vendor.company_id === null, recent_orders: recentPOs });
});
